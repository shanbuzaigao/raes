...
这些文件是 RAES 里“怎么写”的那一层。它们为操作手册里的每一类文件提供固定结构：计划 memo、纳入标准、codebook、审计设计以及 prompt。每个 `{{...}}` 占位符都代表研究者要做的一个决定；占位符旁边的例子是一种填法，取自我的项目，不是必须照做的规范。带占位符的骨架文件可以通过草稿检查，但故意无法通过 `--ready` 检查。
