...
The skill has been checked locally: the copied folder is self-contained and its scripts run. It has been tried three times in Claude Code. The first trial covered the start of stage S0. The second ran every stage, S0 to S12, on a topic from another field, exercise and depressive symptoms in older adults, with a real search, the screening audit, coding, the coding audit, the analysis and a release; what it found became version 0.4.0. The third tried the new tools of 0.4.0 on a small topic, and what it found became 0.4.1. In all three the skill asked for the decisions, wrote its proposals as proposals, and left the decisions to the researcher. A [scripted rehearsal](../examples/raes_rehearsal.md) of the codebook stage on a small invented topic shows what the questions and outputs look like.
