...
made after a release makes verify fail until a new release is created. Exit code 0
when the command succeeds and, for verify, when the working copy matches.
"""
...
def activate(project: Path, name: str) -> int:
    ...
    (project / "releases" / "CURRENT").write_text(f"{name} {sha256_of(manifest)}\n", encoding="utf-8", newline="\n")
