...
- Full text: `python screening/screen_rules_template.py ft records.csv --after-ta <ta folder>/decisions.csv --texts <folder of record_id.txt files> --output <new folder>`. It screens the records kept at the first phase, checks that the first phase covered exactly this records file (every `record_id`, and the input hash in the run's `summary.json`, which has to be next to the decisions file), and stops if any record lacks its text. Records that the title-and-abstract audit confirmed enter from a frozen list passed with `--after-ta-audit`; the title-and-abstract decisions stay as they are. A record whose full text truly cannot be obtained is listed, one identifier per line, in a file passed with `--not-retrieved`; the program skips it and lists it in `summary.json`, so that it is reported as not retrieved, not as excluded.
